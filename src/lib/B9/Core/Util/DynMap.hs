...
singleton :: (Typeable v,v ~ (Value m k),Entry m k)
          => Key m k -> v -> DynMap m
...
insert
  ...
  => Key m k -> Value m k -> DynMap m -> DynMap m
insert k v =
  DynMap .
  Map.insert (toSomeKey k)
             (toDyn v) .
  unDynMap
