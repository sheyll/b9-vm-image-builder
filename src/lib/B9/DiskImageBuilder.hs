...
substImageTarget ::
     forall e. (HasCallStack, Member EnvironmentReader e, Member ExcB9 e)
  => ImageTarget
  -> Eff e ImageTarget
substImageTarget = everywhereM gsubst
  where
    ...
    gsubst =
      mkM substMountPoint `extM` substImage `extM` substImageSource `extM`
      substDiskTarget
...
preferredDestImageTypes src =
  case src of
    ...
    (SourceImage (Image _file fmt _fs) _pt resize) ->
      return
        (nub [fmt, Raw, QCow2, Vmdk] `intersect`
  ...
  let dir =
        let dirRel = takeDirectory path
         in if isRelative dirRel
              then let prefix = fromMaybe "." (b9cfg ^. projectRoot)
                    in prefix </> dirRel
              else dirRel
  liftIO $ do
