...
getArtifactOutputFiles g =
    concatMap getAssemblyOutputFiles <$> map takeAssembly <$>
...
evalArtifactGenerator buildId buildDate b9cfgEnvVars artGen =
    let ag = parseArtifactGenerator artGen
        e =
            CGEnv
                ((buildDateKey, buildDate) :
                 (buildIdKey, buildId) : b9cfgEnvVars)
                []
...
parseArtifactGenerator g =
    case g of
        ...
        EachT keySet valueSets gs -> do
            ...
            mapM_
                ($ mapM_ parseArtifactGenerator gs)
...
writeInstanceGenerator (IID iidStrT) assembly = do
    ...
    iid <-
        either (throwError . CGError) (return . IID) (substE bindings iidStrT)
...
setSGToDirectory toDir (SGConcat e fs p d) = SGConcat e fs p (toDir </> d)

-- | Create the actual target, either just a mountpoint, or an ISO or VFAT
-- image.
createTarget :: InstanceId
             -> FilePath
             -> ArtifactAssembly
             -> B9 [ArtifactTarget]
