...
instance Binary YamlObject where
  ...
  get = YamlObject . either (error . show) id . Yaml.decodeThrow . B.toStrict <$> Binary.getRemainingLazyByteString
    ...
    where
      ...
      readsYamlObject s = [(yamlFromString y, r2) | ("YamlObject", r1) <- lex s, (y, r2) <- reads r1]
        where
          ...
          yamlFromString = either error id . decodeSyntax "HERE-DOC" . E.encodeUtf8 . T.pack

instance Show YamlObject where
  show (YamlObject o) = "YamlObject " <> show (T.unpack $ E.decodeUtf8 $ encode o)
  ...
  fromAST ast =
    case ast of
      ...
      ASTMerge asts -> do
        ys <- mapM fromAST asts
        return (foldl1 (<>) ys)
      ASTEmbed c -> YamlObject . toJSON . T.unpack . E.decodeUtf8 <$> render c
