...
-}
module B9.B9Monad
       (B9, runB9Monad, getConfigParser, getConfig, getBuildId,
        getBuildDate, getBuildDir, getExecEnvType, getSelectedRemoteRepo,
  ...
  where
    withLogFile f =
        maybe
            (f Nothing)
            (\logf ->
                  SysIO.withFile logf SysIO.AppendMode (f . Just))
            (logFile cfg)
...
cmd :: String -> B9 ()
cmd str = do
    traceL "SHELL COMMAND: " str
    execCmd $ shell str

cmdRaw :: FilePath -> [String] -> B9 ()
cmdRaw p args = do
    traceL "COMMAND: " p (show args)
    execCmd $ proc p args
...
logImpl minLevel mh level msg = do
    ...
    when (isJust mh) $
        do SysIO.hPutStr (fromJust mh) lm
