...
spec :: Spec
spec =
    describe "ErlangPropList" $
    do it "implements ConcatableSyntax method decodeSyntax" $
       ...
       it "combines primitives by putting them in a list" $
           let p1 = ErlangPropList (ErlList [ErlAtom "a"])
               ...
               combined =
                   ErlangPropList (ErlList [ErlAtom "a", ErlNatural 123])
           in (p1 <> p2) `shouldBe` combined
       it "combines a list and a primitve by extending the list" $
           let (Right l) =
                   decodeSyntax "" "[a,b,c]." :: Either String ErlangPropList
...
instance Arbitrary ErlPropListTestData where
    arbitrary = do
        ...
        return
            ...
            { plistLeft = zipWith
                  toPair
                  (keysLeftOnly <> keysCommon)
                  valuesLeft
            , plistRight = zipWith
                  toPair
                  (keysRightOnly <> keysCommon)
                  valuesRight
      ...
      where
        ...
        arbitraryPlistEntry =
            toPair <$> arbitraryPlistKey <*> arbitraryPlistValue
